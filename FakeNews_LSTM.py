...
for i in open(path):
    ...
    if len(wordsList) % 100 == 0:
        print '%d words done!' % (len(wordsList))
    if len(wordsList) == 25000:
        break
...
df = pd.read_csv('AICS_Challenge_Data/AICS_Training_Data/training_df.csv')
train_labels = pd.read_csv('AICS_Challenge_Data/AICS_Training_Data/training_label.csv')['label']
...
df = pd.read_csv('AICS_Challenge_Data/AICS_Testing_Data/testing_df.csv')
test_labels = pd.read_csv('AICS_Challenge_Data/AICS_Testing_Data/testing_label.csv')['label']
...
with tf.variable_scope('title_lstm'):
    title_seq, _ = tf.nn.dynamic_rnn(title_lstmCell, title_word_vecs, dtype=tf.float32) # Shape: BatchSize x max_titleSeqLength x lstmUnits
with tf.variable_scope('text_lstm'):
    text_seq, _ = tf.nn.dynamic_rnn(text_lstmCell, text_word_vecs, dtype=tf.float32)   # Shape: BatchSize x max_textSeqLength x lstmUnits
#text_seq = title_seq
...
# Next Batch
    nextBatch_titles, nextBatch_texts, nextBatch_labels = get_nextBatch(batchSize, train_title_idxs, 
                                                                        ...
                                                                        max_titleSeqLength, max_textSeqLength);
    sess.run(optimizer, {title_input: nextBatch_title,
                         text_input: nextBatch_texts,
                         true_label: nextBatch_labels})

#Write summary to Tensorboard
    if (i % 50 == 0):
        summary = sess.run(merged, {title_input: nextBatch_title,
                                    text_input: nextBatch_texts,
                                    true_label: nextBatch_labels})
